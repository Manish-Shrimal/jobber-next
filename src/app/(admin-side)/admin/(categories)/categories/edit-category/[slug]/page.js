...
const APEditCategory = React.memo(() => {
  ...
  const handleColorChange = (e) => {
    ...
    debouncedHandleColorChange(name, value);
  };

  const fetchThemeColors = async () => {
    try {
      const response = await axios.get(BaseApi + "/admin/theme/colors", {
        headers: {
      ...
      if (response.data.status === 200) {
        const { primary_color, secondary_color } = response.data.response;
        setUserData((prev) => ({
          ...prev,
          icon_color: primary_color || "#000000",
          icon_hover_color: secondary_color || "#000000",
        }));
      } else {
        Swal.fire({
          title: "Failed to fetch theme colors",
      ...
      Swal.fire({
        title: "Failed",
        text: "Could not fetch theme colors. Please try again later!",
        icon: "error",
        confirmButtonText: "Close",
      });
      console.log("Error fetching theme colors", error);
    }
  };
  ...
  const handleThemeCheckboxChange = (e) => {
    ...
    if (checked) {
      setUserData((prev) => ({
        ...prev,
        icon_color: userData.primary_color,
        icon_hover_color: userData.secondary_color,
      }));
    } else {
      setUserData((prev) => ({
        ...prev,
  ...
  const getData = async () => {
    try {
      ...
      setSelectedIcon(response.data.response.icon || null);
      setUseWebsiteTheme(!!response.data.response.useWebsiteTheme); // Assuming API returns this
    } catch (error) {
      setLoading(false);
  ...
  const handleClick = async () => {
    try {
      ...
      if (
        newErrors.name ||
        newErrors.icon ||
        newErrors.icon_color ||
        newErrors.icon_hover_color
      ) {
        setErrors(newErrors);
        window.scrollTo(0, 0);
        return;
  ...
  // Memoized Icon Picker Component
  const IconPicker = React.memo(
    ({ filteredIcons, selectedIcon, handleIconSelect, iconColor }) => (
      <div
        className={`form-control ${errors.icon && "input-error"}`}
        style={{
          padding: "10px",
          border: errors.icon ? "1px solid #dc3545" : "1px solid #ced4da",
          borderRadius: "4px",
          maxHeight: "300px",
          overflowY: "auto",
        }}
            ...
            {filteredIcons.map((icon) => (
              <div
                ...
                style={{
                  ...
                  padding: "5px",
                  textAlign: "center",
                  background: selectedIcon === icon ? "#e9ecef" : "transparent",
                  borderRadius: "4px",
                }}
              >
                <FontAwesomeIcon
            ...
            <form className="adminForm mb-5">
              <div className="mb-5 mt-5">
                ...
                >
                  ...
                  <input
                    ...
                    id="icon_color"
                    className={`form-control ${
                      errors.icon_color && "input-error"
                    }`}
                    style={{ height: "38px", padding: "2px" }}
                    name="icon_color"
                    value={userData.icon_color}
                    onChange={handleColorChange}
                    disabled={useWebsiteTheme}
                  />
                  {errors.icon_color && (
                    <div className="text-danger">{errors.icon_color}</div>
                  )}
                  <p>Choose a color for the category icon.</p>
                </motion.div>
                ...
                >
                  ...
                  <input
                    ...
                    id="icon_hover_color"
                    className={`form-control ${
                      errors.icon_hover_color && "input-error"
                    }`}
                    style={{ height: "38px", padding: "2px" }}
                    name="icon_hover_color"
                    value={userData.icon_hover_color}
                    onChange={handleColorChange}
                    disabled={useWebsiteTheme}
                  />
                  {errors.icon_hover_color && (
                    <div className="text-danger">{errors.icon_hover_color}</div>
                  )}
                  <p>Choose a hover color for the category icon.</p>
                </motion.div> */}
                ...
                <div class="d-flex flex-wrap gap-5 mb-2">
                  ...
                  >
                    ...
                    <ChromePicker
                      ...
                      onChangeComplete={(color) =>
                        handleColorChange({
                          target: { name: "icon_color", value: color.hex },
                        })
                      }
                      disableAlpha
                    />
                  ...
                  >
                    ...
                    <p className="mt-2">
                      Choose a hover color for the category icon.
                    </p>
                  </motion.div>
                </div>
              ...
              >
                ...
                <button
                  type="button"
                  className="btn btn-primary button2"
                  onClick={() => router.push("/admin/categories/index")}
                >
                  CANCEL
                </button>
              </motion.div>
